...
def update_data_file(message, data_input, base64_representation, r, s):
    ...
    if data :
        if data.get('message') != message:
            raise ValueError("The message in the data file does not match the provided message.")
...
def generate_ledger_signature(message, account):
    ...
    try:
        ...
        recovered_public_key = signature_obj.recover_public_key_from_msg_hash(hash_bytes)
